RSpec.describe Rack::HealthCheck::CheckFactory do
  class MyCheckClass; end
  class MyOtherCheckClass; def initialize(params); end; end

  let(:definitions) { [] }
  let(:default_checks) { [] }
  subject(:check_factory) { Rack::HealthCheck::CheckFactory.new(definitions, default_checks) }
  ...
  describe 'build_all' do
    context 'with defined checks' do
      ...
      before do
        allow(Rack::HealthCheck::CheckRegistry).to receive(:lookup).with(:my_check).and_return(check_class)
        allow(Rack::HealthCheck::CheckRegistry).to receive(:lookup).with(:my_other_check).and_return(other_check_class)
      end
    ...
    context 'with defined default checks' do
      ...
      let(:http_class) { spy(Rack::HealthCheck::Check::HTTPStatus) }
      before do
        allow(Rack::HealthCheck::CheckRegistry).to receive(:lookup).with(:http).and_return(http_class)
      end
